...
exports.addManualTimeRequest = catchAsync(async (req, res, next) => {
  ...
  const requestApprovalLink = `${process.env.WEBSITE_DOMAIN}/#/home/approvals?tab=manual-time`;
  ...
  SendUINotification(req.t('manualTime.addManualTimenotificationTitle'), req.t('manualTime.addManualTimenotificationMessage', { userName: userName }),
    constants.Event_Notification_Type_Status.manual_time, manager?._id?.toString(), req.cookies.companyId, req);
...
exports.updateManualTimeRequest = catchAsync(async (req, res, next) => {
  ...
  if (updatemanualTimeRequest) {
      ...
      if (req.body.status === "approved") {
        ...
        SendUINotification(req.t('manualTime.updateManualTimenotificationTitle'), req.t('manualTime.updateManualTimenotificationMessage'),
        constants.Event_Notification_Type_Status.manual_time, user?._id?.toString(), req.cookies.companyId, req);
