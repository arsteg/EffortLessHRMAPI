...
const calculatePF = async (req, res) => {
  try {
    ...
    if (!statutoryDetails?.isEmployeeEligibleForPFDeduction) {
      websocketHandler.sendLog(req, '❌ Provident Fund not deducted, skipping LWF', constants.LOG_TYPES.WARN);
      return res.status(200).json({ message: 'Provident Fund not applicable – Provident Fund not deducted' });
    }
    ...
    if (!taxSettings?.isPFDeduction) {
      websocketHandler.sendLog(req, '❌ Provident Fund deduction disabled in settings', constants.LOG_TYPES.WARN);
      return res.status(200).json({ message: 'Provident Fund not applicable – Provident Fund Deduction disabled in settings' });
    }

    // 4️⃣ Calculate the total LWF eligible monthly amount (basic + LWF-affected allowances)
    websocketHandler.sendLog(req, '📊 Calculating total Provident Fund eligible amount', constants.LOG_TYPES.INFO);
    const totalPfEligibleAmount = await getTotalPFEligibleAmount(req, salaryDetails);
    websocketHandler.sendLog(req, `✅ Provident Fund eligible amount: ${totalPfEligibleAmount}`, constants.LOG_TYPES.DEBUG);
    const isEmployeePFCappedAtPFCeiling = taxSettings?.isEmployeePFCappedAtPFCeiling && statutoryDetails?.isEmployeePFCappedAtPFCeiling;
    const isEmployerPFCappedAtPFCeiling = taxSettings?.isEmployerPFCappedAtPFCeiling && statutoryDetails?.isEmployerPFCappedAtPFCeiling;
    const additionalPFContributionInPercentage=statutoryDetails?.additionalPFContributionInPercentage;
   const fixedAmountForYourProvidentFundWage=statutoryDetails?.fixedAmountForYourProvidentFundWage;
     // 6️⃣ Determine the applicable LWF slab and calculate employee/employer contributions
   const { employeeMandatoryPF, employerMandatoryPF, employeeVPF, } = await calculatePFFromCeilingLmit(req,totalPfEligibleAmount, isEmployeePFCappedAtPFCeiling,isEmployerPFCappedAtPFCeiling,additionalPFContributionInPercentage,fixedAmountForYourProvidentFundWage);
    websocketHandler.sendLog(req, `✅ Calculated - Employee: ${employeeMandatoryPF}, Employer: ${employerMandatoryPF}`, constants.LOG_TYPES.DEBUG);
    // 7️⃣ Save LWF contributions to PayrollStatutory (skip if already exists)
    ...
    const currentYear = currentDate.getFullYear();    
      ...
      await storeInPayrollStatutory({
        ...
        payrollUser,
        companyId,
        employeeLWF: employeeMandatoryPF,
        employerLWF: employerMandatoryPF,
        slabId: null,
        month: currentMonth,
        year: currentYear,
        StautoryName,
    ...
    websocketHandler.sendLog(req, '✅ Provident Fund contributions saved successfully', constants.LOG_TYPES.INFO);  

  } catch (err) {
    // ❌ Handle errors and send back an internal server error response
    websocketHandler.sendLog(req, `❌ Provident Fund calculation failed: ${err.message}`, constants.LOG_TYPES.ERROR);
